class Api::V1::UsersController < ApplicationController
  ...
  before_action :authenticate_user!,
                only: %i[update destroy most_interacted_channels most_interacted_users update_favorite_users
                         update_favorite_users subscriptions following check_ownership]
  ...
  def popular
    ...
    users = User.includes(pieces: %i[votes comments]).all
  ...
  def most_interacted_channels
    ...
    subscribed_channels = user.subscriptions.includes(channel: { pieces: %i[comments votes] })
  ...
  def update_favorite_channels
    ...
    if user.present?
      ...
      new_favorite_channels = new_favorite_channels.uniq.reject { |id| id.blank? || !Channel.exists?(id:) }
      ...
      if user.save
        ...
        render json: { favorite_channels: }, status: :ok
      else
  ...
  def update_favorite_users
    ...
    if user.present?
      ...
      new_favorite_users = new_favorite_users.uniq.reject { |id| id.blank? || !User.exists?(id:) }

      new_favorite_users = new_favorite_users.select { |id| user.followees.exists?(id:) }
      ...
      if user.save
        ...
        render json: { favorite_users: }, status: :ok
      else
